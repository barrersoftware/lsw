...
static int safe_kernel_read(kernel_region_t *region, size_t offset, size_t bytes_to_read, void *buffer, size_t buffer_size) {
    ...
    /* Check offset + bytes_to_read does not overflow */
    if (offset > region->size - bytes_to_read) {
        return -1;
    }
...
START_TEST(test_kernel_read_boundary_invariant)
{
    ...
    /* Adversarial (offset, bytes_to_read) pairs encoded as strings for iteration */
    /* We'll use structured test cases instead */
    typedef struct {
        size_t offset;
        size_t bytes_to_read;
        int should_succeed; /* 1 if valid, 0 if must be rejected */
    ...
    for (int i = 0; i < num_cases; i++) {
        ...
        if (cases[i].should_succeed) {
            /* Valid reads must succeed and return correct byte count */
            if (cases[i].bytes_to_read == 0) {
                ...
                /* Verify data integrity: output must match kernel_mem at offset */
            ...
            /* Invalid reads MUST be rejected — this is the security invariant */
            ck_assert_msg(result < 0,
